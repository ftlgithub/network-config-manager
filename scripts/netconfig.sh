...
configure_networkmanager() {
  ...
  # 创建新的静态IP连接
  local cmd="nmcli connection add type ethernet con-name '$connection_name' ifname '$interface' ip4 '$ip/$cidr'"
...
set_ip_linux() {
  ...
  # 检测网络配置方式
  local config_method=$(detect_network_config_method)
  echo "检测到网络配置方式: $config_method"
  
  case "$config_method" in
    "netplan")
      echo "使用Netplan配置永久静态IP..."
      configure_netplan "$interface" "$ip" "$cidr" "$gateway" "$dns"
      ;;
    "networkmanager")
      echo "使用NetworkManager配置永久静态IP..."
      configure_networkmanager "$interface" "$ip" "$cidr" "$gateway" "$dns"
      ;;
    "interfaces")
      echo "使用/etc/network/interfaces配置永久静态IP..."
      configure_interfaces "$interface" "$ip" "$netmask" "$gateway" "$dns"
      ;;
    *)
      echo "警告: 未检测到支持的网络配置方式，使用临时配置"
      # 回退到临时配置
      set_ip_linux_temporary "$interface" "$ip" "$netmask" "$gateway" "$dns"
      return $?
      ;;
  esac
  
  if [[ $? -eq 0 ]]; then
    echo "成功: 已永久配置 $interface 为静态IP: $ip/$cidr"
    ...
    if [[ "$current_ip" == "$ip" ]]; then
      echo "验证成功: 当前IP地址为 $current_ip"
      if [[ -n "$gateway" ]]; then
        local current_gateway=$(ip route show default | awk '{print $3}' | head -n1)
        echo "当前网关: ${current_gateway:-未设置}"
      fi
    else
      echo "警告: IP验证失败，当前IP: ${current_ip:-未获取到}，可能需要重启网络服务"
    fi
  else
    echo "错误: 永久配置失败"
    return 1
