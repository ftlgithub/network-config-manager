...
class NetworkManager {
  ...
  async getMacOSDNS() {
    try {
      ...
      for (const service of services) {
        try {
          const { stdout: dnsOutput } = await execAsync(`networksetup -getdnsservers "${service.trim()}"`);
          const dnsLines = dnsOutput.split('\n')
            .map(line => line.trim())
            .filter(line => line && !line.includes('There aren\'t any DNS Servers'));
          
          allDnsServers.push(...dnsLines);
        } catch {
          // 忽略获取失败的服务
        }
      }
      ...
      // 去除重复的DNS服务器并过滤有效IP
      return [...new Set(allDnsServers)].filter(dns => this.isValidIP(dns));
    } catch {
      return [];
    }
  }
  ...
  async setupSudoPermissions(password) {
    try {
      ...
      // 使用密码执行setup-sudo.sh脚本，并确保SUDO_USER环境变量正确传递
      const command = `echo '${password}' | sudo -S SUDO_USER=${currentUser} ${setupScript} install`;
  ...
  netmaskToCIDR(netmask) {
    // 验证输入格式：必须为形如 A.B.C.D 的字符串
    if (typeof netmask !== 'string') {
      throw new Error('非法的子网掩码');
    }
    ...
    const allowedOctets = new Set([255, 254, 252, 248, 240, 224, 192, 128, 0]);
    let cidr = 0;
    let sawPartial = false; // 是否已经遇到非 255 的八位段
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      // 非 255/0 的情况下，必须在允许的子网掩码片段集合内（确保位模式为前导 1 后跟 0）
      if (!allowedOctets.has(num)) {
        throw new Error('非法的子网掩码');
      }
      ...
      // bits 一定形如 111...1100...00（allowedOctets 保证），firstZero 即为 1 的数量
